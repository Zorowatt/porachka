...
// Edit a Porachka
handlersDashboard.porachkaEdit = function(data,callback){
  // Reject any request that isn't a GET
  if(data.method == 'get'){
    // Prepare data for interpolation
    var templateData = {
      'head.title' : 'Porachka Details',
      'body.class' : 'porachkaEdit'
    };
    // Read in a template as a string
    helpersCommon.getTemplate('porachkiEdit',templateData,function(err,str){
      if(!err && str){
        // Add the universal header and footer
        helpersCommon.addUniversalTemplates(str,templateData,function(err,str){
          if(!err && str){
